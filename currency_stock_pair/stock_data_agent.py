...
# ------------------------------------------------------------------
def load_mcp_config(config_path: str = "stock_config.json"):
...
async def task_executor_groq(task,tool_mapping, tool_defs, model: str = "groq:llama-3.1-8b-instant") -> str:
    ...
    Before you execute first make sure it's that company exists and it has a stock symbol.

    If you a specific duration is mentioned, assume that you need the current date.
    If the duration is lesser than or equal to one day. Assume it's 2 days.

    If no duration, time or date is mentioned. Assume it's for the recent 3 days.
...
async def main():
    ...
    answer = await task_executor_openai("What's the stock price of google?",tool_mapping, tool_defs)
