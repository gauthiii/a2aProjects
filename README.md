...
| Currency & Stocks Pair <br> (A2A + MCP Demo) | A2A connects: Currency Pair Agent ↔ Stock Data Agent. <br> MCP connects: <br> • Currency Pair Agent → Currency MCP Server → Exchange Rate API <br> • Stock Data Agent → Stock MCP Server → Yahoo Finance API. <br><br> **Run Order:** <br> 1. Start Currency MCP Server: <br> `uv run python currency_mcp_server.py` <br> 2. Start Stock MCP Server: <br> `uv run python stock_data_server.py` <br> 3. Start Currency Pair (A2A) Agent: <br> `uv run uvicorn currency_pair_agent:app --port 8081` <br> 4. Start Stock Data (A2A) Agent: <br> `uv run uvicorn stock_data_agent:app --port 8082` <br> 5. Test or Orchestrate Communication: <br> `uv run python test.py` |
